...
export async function getAllAppointments(date, patientName, token) {
  const safePatientName = !patientName || patientName === 'null' ? 'all' : patientName;
  const response = await fetch(`${APPOINTMENT_API}/${date}/${encodeURIComponent(safePatientName)}/${token}`);
  if (!response.ok) {
